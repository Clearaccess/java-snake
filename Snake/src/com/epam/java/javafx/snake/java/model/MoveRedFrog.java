...
public class MoveRedFrog implements IMoveFrog {
    ...
    private void eraseTrace(Frog frog, int[][]field){
        ...
        game.getObserver().update((RedFrog) frog,frog.getX(),frog.getY(),Constants.CLEAR_CELL);
    }

    private void track(Frog frog, int[][]field){
        field[frog.getX()][frog.getY()]=Constants.FROG_INTO_CELL;
        game.getObserver().update((RedFrog) frog,frog.getX(),frog.getY(),Constants.FILL_CELL);
    }
}
